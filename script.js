...
function initContactForm() {
    ...
    contactForm.addEventListener('submit', function (e) {
        ...
        sendEmailWithGoogleAppsScript(formData, submitBtn);
    });
}
...
// Main send function: try JSON POST, fallback to form-encoded POST
function sendEmailWithGoogleAppsScript(formData, submitBtn) {
    submitBtn = submitBtn || document.querySelector('#contactForm button[type="submit"]');
    setButtonPending(submitBtn);
    ...
    .then(response => {
        // If CORS/no-cors makes an opaque response, treat as success (common with some GAS deploys)
        if (response.type === 'opaque') {
            showFormMessage('Message sent. (Server response is opaque due to CORS settings.)', 'success');
    ...
    .then(result => {
        ...
        if (typeof result === 'object' && result.success) {
            ...
            // non-JSON text -> show it and treat as success
            showFormMessage('Message sent. Server returned: ' + (typeof result === 'string' ? result : JSON.stringify(result)), 'success');
            document.getElementById('contactForm').reset();
            setButtonSuccess(submitBtn);
        }
    })
    .catch(err => {
        console.warn('JSON POST failed, falling back to form-encoded POST:', err);
        // fallback will manage button states and errors
        sendEmailWithGoogleAppsScriptAlternative(formData, submitBtn);
    });
}

// Fallback: send as application/x-www-form-urlencoded (URLSearchParams)
function sendEmailWithGoogleAppsScriptAlternative(formData, submitBtn) {
    submitBtn = submitBtn || document.querySelector('#contactForm button[type="submit"]');
    setButtonPending(submitBtn);
    ...
    .then(resp => {
        ...
        if (resp.type === 'opaque') {
            // Treat as success but we can't read response
            showFormMessage('Message sent. (No readable server response.)', 'success');
            document.getElementById('contactForm').reset();
            setButtonSuccess(submitBtn);
            return null;
        }
        if (resp.headers && resp.headers.get && resp.headers.get('content-type') && resp.headers.get('content-type').includes('application/json')) {
            return resp.json();
        }
    ...
    .then(data => {
        ...
        if (typeof data === 'object' && data.success) {
            ...
            showFormMessage('Message sent. Server returned: ' + (typeof data === 'string' ? data : JSON.stringify(data)), 'success');
            document.getElementById('contactForm').reset();
            setButtonSuccess(submitBtn);
        }
    })
...
function validateForm({ name, email, subject, message }) {
    if (!name || !name.trim()) { showFormMessage('Please enter your name.', 'error'); return false; }
    if (!email || !email.trim()) { showFormMessage('Please enter your email address.', 'error'); return false; }
    if (!isValidEmail(email)) { showFormMessage('Please enter a valid email address.', 'error'); return false; }
    if (!subject || !subject.trim()) { showFormMessage('Please enter a subject.', 'error'); return false; }
    if (!message || !message.trim()) { showFormMessage('Please enter your message.', 'error'); return false; }
    return true;
}
...
function showFormMessage(msg, type = 'success') {
    const div = document.getElementById('formMessage') || createFallbackMessageContainer();
    div.textContent = msg;
    div.className = `mt-4 text-center alert ${type === 'success' ? 'alert-success' : 'alert-danger'}`;
    setTimeout(() => {
        div.textContent = '';
...
function createFallbackMessageContainer() {
